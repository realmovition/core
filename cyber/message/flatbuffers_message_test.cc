...
TEST(FlatBufferMessageTest, traits_use_concrete_type_name_when_present) {
  auto buf = BuildChatterBuffer(10, "type-name");
  FlatBufferMessage msg(kChatterMessageType, buf.data(), buf.size());
  FlatBufferMessage generic_msg;

  EXPECT_EQ(MessageType(msg), kChatterMessageType);
  EXPECT_EQ(GetMessageName(msg), kChatterMessageType);
  EXPECT_EQ(MessageType<FlatBufferMessage>(), FlatBufferMessage::TypeName());
  EXPECT_EQ(GetMessageName(generic_msg), FlatBufferMessage::TypeName());
}

TEST(FlatBufferMessageTest, serialize_to_hc_preserves_concrete_type_name) {
  auto buf = BuildChatterBuffer(11, "header-roundtrip");
  FlatBufferMessage src(kChatterMessageType, buf.data(), buf.size());

  std::vector<uint8_t> scratch(FullByteSize(src));
  ASSERT_TRUE(
      SerializeToHC(src, scratch.data(), static_cast<int>(scratch.size())));

  FlatBufferMessage dst;
  ASSERT_TRUE(ParseFromHC(scratch.data(), static_cast<int>(scratch.size()),
                          &dst));

  EXPECT_EQ(dst.type_name(), kChatterMessageType);
  const auto* chatter = dst.GetRoot<examples::proto::Chatter>();
  ASSERT_NE(chatter, nullptr);
  EXPECT_EQ(chatter->seq(), 11u);
  EXPECT_STREQ(chatter->content()->c_str(), "header-roundtrip");
}
