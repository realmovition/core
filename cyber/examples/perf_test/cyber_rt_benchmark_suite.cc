...
std::string MakeChannelName(const BenchmarkCaseConfig& config, uint64_t suffix) {
  ...
  oss << "perf/" << ToString(config.scenario) << "/" << ToString(config.coverage)
      << "/" << ToString(config.topology) << "/p" << config.publishers << "_s"
      << config.subscribers << "_f" << config.frequency_hz << "_b"
...
class BenchmarkSuiteRunner {
 ...
 private:
  ...
  bool RunShmZeroCopyProbeViaWorkers() {
    ...
    result.success = published && received && loan_supported &&
                     borrowed_messages > 0 && copy_count == 0;
    if (!result.success) {
      result.error_message =
          "SHM zero-copy probe failed (publish/receive/loan/copy_count criteria)";
    }
  ...
  BenchmarkCaseResult RunSingleCaseViaWorkers(const BenchmarkCaseConfig& config) {
    ...
    result.shm_profile_recorded = false;
    std::ostringstream notes;
    notes << "real_multi_process=true"
