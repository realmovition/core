...
def print_param_srv():
    with parameter.Parameter("author_name", "WanderingEarth") as param1, \
        parameter.Parameter("author_age", 5000) as param2, \
        parameter.Parameter("author_score", 888.88) as param3, \
        cyber.Node(PARAM_SERVICE_NAME) as test_node:
        with parameter.ParameterServer(test_node) as srv, \
            parameter.ParameterClient(test_node, PARAM_SERVICE_NAME) as clt:
